...
class MesonApp:
    ...
    def gen_source(self, env: environment.Environment, target):
        ...
        gti: GCTargetInfo = GCTargetInfo.loadfrom(env, target)

        # 헤더파일
        for d_file in gti.final_obj_list:
            if d_file in gti.generated_object_files: continue
            d_path = os.path.abspath(os.path.join(env.get_build_dir(), d_file + ".d"))
            ...
            # _file = os.path.relpath(d_path, env.get_build_dir())
        # for t in obj_list if t not in generated_object_files]
