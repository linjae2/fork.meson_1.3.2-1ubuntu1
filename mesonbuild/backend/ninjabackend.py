...
''')
    ...
    def generate_target(self, target):
        ...
        compiled_sources = []
        source2object = {}
        commandArgs = OrderedSet()
        source2command: T.MutableMapping[str, CompilerArgs] = OrderedDict()
        name = target.get_id()
        ...
        if t_filename == "tests_qemumonitorjsontest":
            pass
        ...
        for rel_src in generated_sources.keys():
            ...
            if self.environment.is_source(rel_src):
                ...
                generated_object_files.append(rel_src)
        ...
        for src in generated_source_files:
            if self.environment.is_llvm_ir(src):
                ...
                o, s, c = self.generate_single_compile(target, src, True,
                                                    order_deps=header_deps)
            compiled_sources.append(s)
            source2object[s] = o
            obj_list.append(o)
            if isinstance(c, CompilerArgs): source2command[s] = c
        ...
        for src in vala_generated_source_files:
            ...
            o, s, _ = self.generate_single_compile(target, src, 'vala', [], header_deps)
            obj_list.append(o)
        ...
        for src in target_sources.values():
            if not self.environment.is_header(src) or is_compile_target:
                if self.environment.is_llvm_ir(src):
                    ...
                    o, s, c = self.generate_single_compile(target, src, False, [],
                                                        header_deps + d_generated_deps + fortran_order_deps,
                                                        fortran_inc_args)
                    obj_list.append(o)
                    compiled_sources.append(s)
                    source2object[s] = o
                    commandArgs.update(c)
                    if isinstance(c, CompilerArgs): source2command[s] = c
        ...
        elem, c = self.generate_link(target, outname, final_obj_list, linker, pch_objects, stdlib_args=stdlib_args)
    ...
    def generate_swift_target(self, target):
        ...
        if isinstance(target, build.StaticLibrary):
            elem, _ = self.generate_link(target, self.get_target_filename(target),
...
https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47485'''))
    ...
    def generate_single_compile(self, target: build.BuildTarget, src,
        ...
        return (rel_obj, rel_src.replace('\\', '/'), commands)
